...
export class DashCRUDService {
  ...
  createEntry(uploadInfo: CRUDdata, type: string) {
    ...
    return this.CRUDserv.uploadImages(uploadInfo.NewImageLinks, uploadInfo.ImageBlobs)
    .then(links => {
      if (links[0]) {
  ...
  editEntry(ID: string) {
    ...
    .subscribe(uploadInfo => {
      ...
      return this.CRUDserv.editImages(uploadInfo.NewImageLinks,
